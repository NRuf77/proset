...
%
\begin{equation}
\hat{q}_c(y|x):=H_{\hat{s}^{-1}_N}(y-\bar{y}_N)+\sum_{b=1}^c\sum_{j=1}^{J_b}H_{u_b}(y-y_{s_{b,j}})w_{b,j}G_{v_b}(x-x_{s_{b,j}})\label{eq_hatqcxy}
\end{equation}
%
...
\end{equation}
%
In particular, $\hat{p}_0(y|x)=H_{\hat{s}^{-1}_N}(y-\bar{y}_N)$ and $\hat{p}_B(y|x)=\hat{p}(y|x)$.
%
\endinput
