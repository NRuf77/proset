...
See (\ref{eq_nll_classifier}), (\ref{eq_l_partial_v}), and (\ref{eq_l_partial_w}) for the expressions belonging to the proset classifier.
...
%
\begin{enumerate}
\item To evaluate the objective function (\ref{eq_nll_classifier}), we need to compute the squared weighted Euclidean distance between every pair of points where one is a sample reserved for scoring and the other a potential prototype.
