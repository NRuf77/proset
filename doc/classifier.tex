...
The likelihood function shown below was modified in version 0.5.0 of the algorithm.
See Section \ref{sec_updated_candidate_sampling} `Updated candidate sampling' for details.
}}\end{center}
%
\begin{center}\fbox{\parbox{0.95\textwidth}{\small
Before version 0.6.0 of the algorithm, the role of each $\alpha$ was reversed, i.e., a large value indicated a dominant $L_2$ penalty.
This has been changed throughout the source code and documentation to be consistent with the reference literature.
}}\end{center}
%
...
%
\begin{align}
f(v_c,\{w_{c,j}\}_j|\{(x_n,y_n)\}_n)
&=-\frac{1}{N}\sum_{k=0}^{K-1}\frac{N_k}{N_k-J_{c,k}}\sum_{n\in T_c}
\1_{\{k\}}(y_n)\log(\hat{p}_{c,k}(x_n))\label{eq_nll_classifier}\\
...
\end{itemize}
%
Given a set of prototypes $S_c$, the objective function (\ref{eq_nll_classifier}) is maximized subject to $v_{c,d}\geq0$ and $w_{c,j}\geq0$.
...
This means that the sum of terms in (\ref{eq_nll_classifier}) is replaced by a weighted sum normalized by total weights instead of observation counts.
...
\end{remark}
%
To gain the full advantage of using L-BFGS-B, we need to compute the gradient of (\ref{eq_nll_classifier}) analytically.
...
\left(\1_{\{k\}}(y_{s_{c,i}})w_{c,i}G_{v_c}(x-x_{s_{c,i}})\right)}
{\hat{q}_{c,k}(x)}
\label{eq_p_partial_w}\\
&-\frac{\frac{\partial}{\partial w_{c,i}}
\left(w_{c,i}G_{v_c}(x-x_{s_{c,i}})\right)}
{\sum_{l=0}^{K-1}\hat{q}_{c,l}(x)}\notag\\
...
%
\begin{align}
\frac{\partial}{\partial v_{c,d}}f(v_c,\{w_{c,j}\}_j|\{(x_n,y_n)\}_n)
&=-\frac{1}{N}\sum_{k=0}^{K-1}\frac{N_k}{N_k-J_{c,k}}\sum_{n\in T_c}
\1_{\{k\}}(y_n)\frac{\partial}{\partial v_{c,d}}\log(\hat{p}_{c,k}(x_n))
\label{eq_l_partial_v}\\
&+\lambda((1-\alpha_v)v_{c,d}+\alpha_v)\notag\\
\frac{\partial}{\partial w_{c,j}}f(v_c,\{w_{c,j}\}_j|\{(x_n,y_n)\}_n)
&=-\frac{1}{N}\sum_{k=0}^{K-1}\frac{N_k}{N_k-J_{c,k}}\sum_{n\in T_c}
...
%
\begin{center}\fbox{\parbox{0.95\textwidth}{\small
The strategy for selecting prototypes described below was modified in version 0.5.0 of the algorithm.
See Section \ref{sec_updated_candidate_sampling} `Updated candidate sampling' for details.
}}\end{center}
%
...
%
\begin{center}\fbox{\parbox{0.95\textwidth}{\small
Further experimentation with larger data sets showed that there is a tradeoff between $\lambda_v$ and $B$.
Optimizing $\lambda_v$ for a single batch and then adding more batches can lead to modest overfitting.
Thus, if we choose $B$ after the penalty as proposed below, cross-validation is likely to select a single batch even if multiple batches would yield a better model.
To avoid this, we implemented a different fit strategy starting with version 0.4.0 of the algorithm.
This focuses on jointly optimizing $\lambda_v$ and $B$ while keeping all other parameters at the recommended defaults.
See Section \ref{sec_updated_fit} `Updated fit strategy' for details.
}}\end{center}
%
...
\subfloat[\textbf{Both stages}]{\includegraphics[width=0.99\textwidth, trim={0.5cm 0.5cm 0.5cm 0.3cm}, clip]{figures/iris_2f_dominant_l1_1d_search.pdf}}
\end{center}
\end{figure}
%
...
\clearpage
%
\section{Updated candidate sampling}
\label{sec_updated_candidate_sampling}
%
\begin{center}\fbox{\parbox{0.95\textwidth}{\small
Except where noted, the results in this section were generated using version 0.5.0 of Python package \texttt{proset}.
Different versions of \texttt{proset} or the underlying compute libraries may yield results that are qualitatively similar but not identical.
}}\end{center}
%
...
To generalize this approach to regression, we want to choose a data-adaptive threshold on the likelihood (predicted density at the candidate point).
For a more consistent procedure, we apply a similar idea to classification:
...
\end{enumerate}
%
Another change implemented as part of this update is how re-weighting of samples in the likelihood function (\ref{eq_nll_classifier}) is handled.
...
This is done to further minimize the distortion introduced by the non-stratified candidate selection.\par
%
\begin{table}
